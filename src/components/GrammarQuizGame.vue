...
const list = shuffleArray(grammarQuestions).slice(0, 10)
const total = list.length
const index = ref(0)
const score = ref(0)
const feedback = ref('请选择答案 · Choose an answer')
...
function pick(option: string): void {
  ...
  setTimeout(() => {
    if (index.value < total - 1) {
      index.value += 1
      feedback.value = '请选择答案 · Choose an answer'
      locked.value = false
      return
    }
